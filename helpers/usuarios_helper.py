...
def listar_usuarios(base_url, params=None):
    return requests.get(f"{base_url}/usuarios", params=params)
...
def buscar_usuario(base_url, user_id):
    return requests.get(f"{base_url}/usuarios/{user_id}")


def atualizar_usuario(base_url, user_id, payload):
    return requests.put(f"{base_url}/usuarios/{user_id}", json=payload)
